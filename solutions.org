...
** 3
   ...
   *Lemma*: Given two manifolds $M$ and $N$ of the same dimension and
   $f$ be a bijective continuous function between these two manifolds,
   then $f$ is a homeomorphism.

   /Proof./ It is enough to show that $f$ is an open mapping, i.e., if
   ...
   $$\R^n \rightarrow W \rightarrow U' \rightarrow f(U') \rightarrow V' \rightarrow \R^n.$$
   ...
   of $V'$, say $V$. One can see that $V$ is the required open set. $\blacksquare$
...
** 7
   ...
   2. Suppose that a $n$ dimensional vector bundle over $E$, say $B$,
      ...
      \rightarrow \E^n(\R^n)$ that is also an isomorphism
      between fibres. Let us denote the the isomorphism between fib re
      at $p$ and $\R^n$ by $\phi_p \colon \R^n \rightarrow
      \pi^{-1}(p)$. If $\{e_1, \cdots, e_n\}$ represents the standard
      basis over $\R^n$, then we can define sections $s_1, \cdots,
      s_n \colon E \rightarrow B$ in the following way:
